# 1. Vacuum-decay
A quantum false vacuum decay simulation.

[![Vacuum decay](./Logos/Vacuum_decay.png)](https://www.youtube.com/watch?v=cR_HJzRzr_Y)
...
# 3. Vacuum_decay installation
## 3.1 Cloning Vacuum_decay
...
`git clone https://github.com/HoneyCodes/Vacuum_decay.git` \
\
This will create the `Vacuum_decay` directory.
...
### **Command shell**:
Navigate to the `Vacuum_decay` directory, create a configuration script file called `configure_linux` (e.g. at the command shell type: `touch configure_linux`) and edit it by adding the following command lines:\
`cmake -B"./cmake" -H"./" \`\
...
### **VScode:**
Alternatively, open a **VScode shell terminal** and go to the `Vacuum_decay` directory, then create a `.vscode` hidden directory:\
`mkdir .vscode`\
...
### **Visual Studio**:
Go to the `Vacuum_decay` directory: create and edit a `CMakeSettings.json` file in it, then fill it with the following information:\
`{`\
&nbsp;&nbsp;`"configurations": [`\
&nbsp;&nbsp;&nbsp;&nbsp;`{`\
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"name": "x64-Release",`\
...
### **VScode**:
Go to the `Vacuum_decay` directory and create a `.vscode` hidden directory:\
`mkdir .vscode`\
\
...
\
To edit the *project* settings, open `settings.json` file in the `.vscode` you created for Neutrino (the hidden directory inside the `Vacuum_decay` directory) and put the same lines in it. This will set Uncrustify as code formatter (together with the configuration file we provide) only for the Neutrino project.
